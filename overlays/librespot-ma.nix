final: prev: {
  ...
  # Replace the default librespot in nixpkgs with the MA fork so anything
  # depending on pkgs.librespot picks up the forked binary (supports --check-auth).
  librespot = final.librespot-ma;
}
